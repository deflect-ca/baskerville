...
from baskerville.features.feature_payload_size_log_average import \
    FeaturePayloadSizeLogAverage, FeatureRequestTotal

from tests.unit.baskerville_tests.helpers.spark_testing_base import \
    FeatureSparkTestCase


class TestSparkPayloadSizeLogAverage(FeatureSparkTestCase):

    def setUp(self):
        super(TestSparkPayloadSizeLogAverage, self).setUp()
        self.feature = FeaturePayloadSizeLogAverage()
    ...
    def test_compute_single_record(self):
        ...
        expected_df = sub_df.withColumn(
            ...
            F.lit(np.log(1000. + 1.)).cast('float')
        )
    ...
    def test_compute_multiple_records_different_depth(self):
        ...
        expected_value = np.mean([np.log(1001.), np.log(2001.), np.log(3001.)])
        expected_df = sub_df.withColumn(
            self.feature.feature_name,
            F.lit(expected_value).cast('float')
        )
        expected_df = self.schema_helper(
            expected_df, result.schema, [self.feature.feature_name]
        )
