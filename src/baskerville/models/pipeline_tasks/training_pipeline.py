...
from baskerville.models.pipeline_tasks.tasks import GetDataPostgres, Train, GetDataFromStorage
...
def set_up_training_pipeline(config: BaskervilleConfig):
    ...
    if config.engine.training.load_from_storage:
        training_tasks = [
            GetDataFromStorage(  # or any other source
                config,
                from_date=datetime.strptime(data_params.get('from_date'),
                                            config.engine.datetime_format),
                to_date=datetime.strptime(data_params.get('to_date'),
                                          config.engine.datetime_format),
                steps=[
                    Train(config)
                ]),
