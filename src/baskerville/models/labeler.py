...
class Labeler(object):
    ...
    def __init__(self, db_config, spark, s3_path, regular_traffic_before_attack_in_minutes=30, logger=None):
        super().__init__()
    ...
    def load_dataset(self, query, engine):
    ...
    def label_attack(self, attack, engine):
        self.logger.info(f'Labeling incident id = {attack.id}, target={attack.target} ...')

        df_attack = self.load_dataset(f'select * from request_sets where target=\'{attack.target}\' and '
                                      f'stop >= \'{attack.start.strftime("%Y-%m-%d %H:%M:%S")}Z\' and '
                                      f'stop < \'{attack.stop.strftime("%Y-%m-%d %H:%M:%S")}Z\'', engine)

        regular_start = attack.stop - datetime.timedelta(minutes=self.regular_traffic_before_attack_in_minutes)
        df_regular = self.load_dataset(f'select * from request_sets where target=\'{attack.target}\' and '
    ...
    def label(self):
        ...
        try:
            ...
            for attack in attacks:
                self.label_attack(attack, engine)
    ...
    def load_request_sets(self, query):
    ...
    def save_df_to_s3(self, df, attack_id):
    ...
    def save_attack(self, attack):
        ...
        df = self.load_request_sets(query)
        if len(df.head(1)) == 0:
        ...
        self.save_df_to_s3(df, attack.id)
