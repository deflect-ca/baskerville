        ...
        if categoryNames is None:
            if node["splitType"] == "categorical":
                ...
                for i in range(0, len(node["leftCategories"]), 5):
                    list_of_categories_5_by_5.append(",".join(
                        [str(j) for j in node["leftCategories"][i:i + 5]]))
                categories = "categories# " + "{" + "\\n".join(
                    str(n) for n in list_of_categories_5_by_5) + "}"
        else:
            if node["splitType"] == "categorical":
                try:
                    category_names = dict(
                        enumerate(categoryNames[feature_name]))
                    list_of_categories_5_by_5 = []
                    for i in range(0, len(node["leftCategories"]), 5):
                        list_of_categories_5_by_5.append(",".join(
                            [category_names[j] for j in
                             node["leftCategories"][i:i + 5]]))
                    categories = "{" + "\\n".join(
                        n for n in list_of_categories_5_by_5) + "}"
                except KeyError:
                    list_of_categories_5_by_5 = []
                    for i in range(0, len(node["leftCategories"]), 5):
                        list_of_categories_5_by_5.append(",".join(
...
def relations_to_str(node, featureNames=None, categoryNames=None,
                     classNames=None,
                     numClasses=None, nodeList=None, filled=True,
                     roundLeaves=True,
                     color_brew=None):
    """
    ...
    while len(nodes_to_explore) > 0:
        ...
        relations.append(node_to_str(current_node, featureNames, categoryNames,
                                     classNames, numClasses, nodeList, filled,
                                     roundLeaves, color_brew)
                         + "->"
                         + node_to_str(current_node["leftChild"], featureNames,
                                       categoryNames,
                                       classNames, numClasses, nodeList,
                                       filled, roundLeaves, color_brew)
        ...
        relations.append(node_to_str(current_node, featureNames, categoryNames,
                                     ...
                                     roundLeaves, color_brew)
                         + "->"
                         + node_to_str(current_node["rightChild"],
                                       featureNames, categoryNames,
                                       classNames, numClasses, nodeList,
                                       filled, roundLeaves, color_brew)
...
def generate_tree_json(DecisionTreeClassificationModel, withNodeIDs=False):
    ...
    json_tree = sc._jvm.com.vfive.spark.ml.SparkMLTree(
        DecisionTreeClassificationModel._java_obj).toJsonPlotFormat()

    if withNodeIDs:
        json_tree = dumps(
            add_node_ids(loads(json_tree, object_pairs_hook=OrderedDict)),
            indent=2)

    return json_tree


def export_graphviz(DecisionTreeClassificationModel, featureNames=None,
    ...
    """
    tree_dict = loads(
        generate_tree_json(DecisionTreeClassificationModel, withNodeIDs=False))
